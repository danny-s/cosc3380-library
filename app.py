...
from web import HTTPResponse, HTTPServerFailure, response, HTTPRequest, DEFAULT_BLOCK_SIZE, MultiField
...
class Application:
    ...
    def resolve(self, environ: dict) -> Callable[[HTTPRequest], HTTPResponse]:
        ...
        try:
            ...
            if url is None:
                raise HTTPServerFailure(404, f"URL {environ['PATH_INFO']} DOES NOT APPEAR TO MATCH ANY REGISTERED URLS!")

        try:
            handler = url.get(environ['REQUEST_METHOD'], None) or url[None]
        except KeyError:
                raise HTTPServerFailure(404, f"URL {environ['REQUEST_METHOD']} DOES NOT APPEAR TO HAVE A "
