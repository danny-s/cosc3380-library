...
class MultiField:
    ...
    def clear_field(self, key):
        key = key.lower()
        pos = [
            i for i in range(len(self._fields) - 1, -1, -1) if self._fields[i] == key
        ]
        for i in pos:
...
class HTTPServerFailure(Exception):
    def __init__(self, code: int = 500, message: str = None, *a, **kw):
        self.code = code
        self.message = (
            message if message is not None else f"{code} Internal Server Error"
        )
    ...
    def templated_static_response(request: HTTPRequest):
        ...
        if ".." in url_path:
            return error_response(
                request,
                "403 Forbidden",
                "Parent paths (..) not allowed for static urls.",
            )
        ...
        if not file_path.is_file():
            return error_response(
                request, "404 Not Found", "Requested file was not found on this server."
            )
